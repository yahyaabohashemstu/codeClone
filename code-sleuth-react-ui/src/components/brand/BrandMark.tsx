 ...
 *
 * The C is the lens and the handle completes the instrument; the L is what the
 * lens finds. On the bench the lens prints in the primary text ink and the L
 * in the signal colour (Brand/Mark in the design file). Pass explicit colours
 * only where the surface is not the bench.
 *
...
export function BrandMark({
  className,
  lens = "var(--text-primary)",
  letter = "var(--signal-base)",
  title,
}: {
  className?: string;
...
export function BrandLockup({
  ...
  return (
    <span className={cn("flex items-center gap-2.5", className)}>
      ...
      <span className={cn("ui-wordmark whitespace-nowrap text-txt-primary", wordClassName)}>Clone Lens</span>
    </span>
  );
}
